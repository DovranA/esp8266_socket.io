...
const byte relay3 = 5; 
bool relay3Flag = 0;
const byte relay3Inp = 15;  
int deviceIds[]={-2,-1,0};
int pinArr[]={relay1, relay2, relay3};
const String deviceType = "3relay";
const int serial = 110;
SocketIoClient sc;
void setup() {
   Serial.begin(115200);
  ...
  bool res = wifiManager.autoConnect("socket3relay");
...
void loop(){
  ...
  if(switcher2 == 0 && relay2Flag == 1){
   ...
   boolean switcher3 = digitalRead(relay3Inp);
  if(switcher3 == 1 && relay3Flag == 0 ){
      relay3Flag = 1;
      digitalWrite(relay3, HIGH); 
      deviceEmit(deviceIds[2], 1);    
  }
  if(switcher3 == 0 && relay3Flag == 1){
      relay3Flag = 0;
      digitalWrite(relay3, LOW); 
      deviceEmit(deviceIds[2], 0); 
  }
  sc.loop();
}
